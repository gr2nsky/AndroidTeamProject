...
<%
  ...
  String q2 = "FROM (SELECT * FROM reservation WHERE designer_no = ?) r, ";
  String q3 =	"(SELECT r.no, s.title FROM styling s, reservation r WHERE r.styling_no = s.no) st, ";
  String q5 = "(SELECT r.no, u.name FROM user u, reservation r WHERE r.user_no = u.no) u, ";
  String q6 = "(SELECT r.no, u.name FROM designer d, user u, reservation r WHERE r.designer_no = d.dno AND d.user_no = u.no) d ";
  String q7 = "WHERE r.no = st.no AND r.designer_no = d.no  AND r.user_no = u.no ";
  String q8 = "ORDER BY r.reservationDate DESC, r.reservationTime DESC";
  String query = q1 + q2 + q3 + q4 + q5 + q6 + q7 + q8;
...
%>
      ...
      "no" : "<%=rs.getInt(1) %>",
			...
			"designerName" : "<%=rs.getInt(3) %>",
			"userName" : "<%=rs.getInt(4) %>",
